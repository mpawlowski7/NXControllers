...
VERSIONS=($(dkms status 2>/dev/null | sed -E -n 's/hid-nintendo, ([0-9]+.[0-9]+).*/\1/ p' | sort -u))
...
do
    ...
    dkms remove -m hid-nintendo -v "$instance" --all

    echo "Rmoving $instance folder from /usr/src"
    rm --recursive "/usr/src/hid-nintendo-$instance/"
done